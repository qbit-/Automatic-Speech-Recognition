...
if gpus:
    ...
    print(f"hvd_local_rank: {local_rank}")
...
def train_model(filename, dataset_idx, n_blocks=1, is_mixed_precision=True,
    ...
    dataset = asr.dataset.Audio.from_csv(dataset_idx, batch_size=batch_size * hvd.size(),
                                         use_filesizes=True)
    ...
    if val_dataset_idx:
        val_dataset = asr.dataset.Audio.from_csv(
            val_dataset_idx,
            batch_size=batch_size * hvd.size(), use_filesizes=True)

    #opt_instance = tf.optimizers.Adam(0.001 * hvd.size(), beta_1=0.9, beta_2=0.999)
    opt_instance = tfa.optimizers.NovoGrad(0.001 * hvd.size(), beta_1=0.95,
    ...
    schedule = tf.keras.experimental.CosineDecayRestarts(
        0.001 * hvd.size(), 10, t_mul=2.0, m_mul=0.7, alpha=0.0
    )
